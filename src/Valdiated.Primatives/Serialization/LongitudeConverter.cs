...
/// <summary>
/// JSON converter for Longitude value objects.
/// Serializes as an object containing both the longitude value and decimal places configuration.
/// </summary>
public class LongitudeConverter : JsonConverter<Longitude>
{
    /// <summary>
    /// Reads and converts the JSON to Longitude.
    /// </summary>
    public override Longitude? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (reader.TokenType == JsonTokenType.StartObject)
        {
            ...
            var (result, longitude) = Longitude.TryCreate(value, decimalPlaces, "Longitude");

            if (!result.IsValid)
            {
                var errors = string.Join(", ", result.Errors.Select(e => e.Message));
                throw new JsonException($"Failed to deserialize Longitude: {errors}");
            }

            return longitude;
        }

        throw new JsonException("Expected JSON object for Longitude");
    }

    /// <summary>
    /// Writes the Longitude as JSON.
    /// </summary>
